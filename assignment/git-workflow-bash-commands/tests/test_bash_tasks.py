...
def test_marker_file_exists_and_matches_expected_content():
    marker_path = REPO_ROOT / "sandbox" / "notes" / "marker.txt"
    text = read_text(path=marker_path).replace("\r\n", "\n")
    assert text == "done\n" or text == "done", "Create sandbox/notes/marker.txt with contents 'done'"
