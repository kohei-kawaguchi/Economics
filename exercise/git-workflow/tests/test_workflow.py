...
This grader is intentionally loose: as long as commits are minimal and complete
and messages are relevant, it is acceptable.
"""
...
def get_exercise_pathspec() -> str:
    git_root = get_git_root()
    exercise_root = Path(__file__).parent.parent.resolve()
    rel = os.path.relpath(exercise_root, start=git_root)
    return rel


def get_commits():
    """Get list of commits (hash, message, files changed) for this exercise."""
    pathspec = get_exercise_pathspec()
...
def test_has_commits():
    ...
    assert commits, "No commits found for this exercise."
