...
foreach::foreach(i = 1:length(theta_nonlinear), 
 ...
 for (j in 1:length(tm)) {
   ...
   objective_fn[j] <- 
     Economics::GMM_objective_A4(theta_nonlinear_search, delta, df_share_smooth, Psi, X, M, V, kappa, lamda)
 }
...
```{r}
estimate <- optim(par = theta_nonlinear, 
                  fn = Economics::GMM_objective_A4,
                  delta = delta,
